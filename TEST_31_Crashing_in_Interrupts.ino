...
//Digital interrupt       HIGH might not work - Since the PIR's stay HIGH for a set time I was hoping to use that
  attachInterrupt(digitalPinToInterrupt(0), MomsPIRs, RISING);     //Set pin 2 to turn LED's on red medium brightness PIR sensors in HIGH state
  attachInterrupt(digitalPinToInterrupt(1), DadsPIRs, RISING);     //Set pin 3 to turn on all lights bright white when button is pushed
  attachInterrupt(digitalPinToInterrupt(2), Panic, RISING);      //Set pin 3 to turn on all lights bright white when button is pushed
  attachInterrupt(digitalPinToInterrupt(7), PwrOut, RISING);     //Set pin 3 to turn on all lights bright white when button is pushed
...
void MomsPIRs()     //Probably connected to FOOT PIR also
  {
      Serial.println(" aFF") ;            //Turn LEDs ON
      irsend.sendNEC(0xFF02FD, 32) ;
      Serial.println(" DIY1") ; 
      irsend.sendNEC(0xFF30CF, 32);       //Set LEDs to medium pink - DIY 1
  }

//Interrupt 1 - LEDs on my side only purple medium
void DadsPIRs()
  {                       
      Serial.println(" bFF") ;            //Turn LEDs ON
      irsend.sendNEC(0xFF02FD, 32) ;
      Serial.println(" DIY2") ; 
      irsend.sendNEC(0xFFB04F, 32);       //Set LEDs to medium pink - DIY 2
  }

//Interrupt 2 - LEDs bright white
void Panic()
  {                     
      Serial.println(" cFF") ;            //Turn LEDs ON
      irsend.sendNEC(0xFF02FD, 32) ;
...
void PwrOut()
{
      ...
      Serial.println(" dFF") ;            //Turn LEDs ON
      irsend.sendNEC(0xFF02FD, 32) ;
      Serial.println(" FADE") ;           //Choose one of the FADEs
