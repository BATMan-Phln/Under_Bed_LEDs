...
//Digital interrupt       HIGH might not work - Since the PIR's stay HIGH for a set time I was hoping to use that
  attachInterrupt(digitalPinToInterrupt(0), MomsPIRs, HIGH);     //Set pin 2 to turn LED's on red medium brightness PIR sensors in HIGH state
  attachInterrupt(digitalPinToInterrupt(1), DadsPIRs, HIGH);     //Set pin 3 to turn on all lights bright white when button is pushed
  attachInterrupt(digitalPinToInterrupt(2), Panic, RISING);      //Set pin 3 to turn on all lights bright white when button is pushed
  attachInterrupt(digitalPinToInterrupt(3), PwrOut, RISING);     //Set pin 3 to turn on all lights bright white when button is pushed
...
void MomsPIRs()     //Probably connected to FOOT PIR also
  {
      Serial.println(" OFF") ;            //Turn LEDs ON
