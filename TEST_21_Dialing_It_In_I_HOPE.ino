...
volatile  int pir1 = 2;       //Pick some inputs for the 3 PIR's
// COMBINED WITH MomsPIRs - volatile  int pir2 = ;    //Might have to put all PIR's on one Interrupt pin (2)
volatile  int pir3 = 0;
...
//Interrupt 0 - LEDs red medium
void MomsPIRs()
  {
      irsend.sendNEC(0xFF30CF, 32);      //Turns LEDs on medium redish - DIY 1
  }

//Interrupt 1 - LEDs on my side only red medium
void DadsPIRs()
  {
      irsend.sendNEC(0xFFB04F, 32);      //Turns LEDs on my side only on medium redish - DIY 2
  }
...
//Interrupt 3 - LEDs fadiing up/down red
void PwrOut()
{

      //When power goes out a relay switches to battery and turns all lights fadiing up/down red - DIY 4

      irsend.sendNEC(0xFF10EF, 32);      //Turns LEDs on fadiing up/down red
